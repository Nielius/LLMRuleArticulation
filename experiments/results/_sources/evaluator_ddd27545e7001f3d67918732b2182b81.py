...
class TaskEvaluator:
    ...
    def evaluate(self, test_data: list[LabelledInput]) -> EvaluationReport:
        ...
        output = [
            self.get_true_or_false_response(self.evaluation_prompt(input))
    ...
    def evaluation_prompt(self, input: str) -> list[ChatCompletionMessageParam]:
        return messages_for_single_prompt(
            self.task.get_system_prompt(), format_labelled_input(input, "???")
        )
